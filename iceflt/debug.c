...
) 
{
    ...
    switch (Status)
    {
        ...
        case STATUS_SXS_CORRUPT_ACTIVATION_STACK:
            ...
            //
            // Filter Manager specific error codes
            //
        ...
        case 0xC01C0020: return "STATUS_FLT_NO_WAITER_FOR_REPLY";
            ...
            //
            // WDF / KMDF specific error codes
            //
        case 0x40200000: return "STATUS_WDF_QUEUED";
        case 0x40200002: return "STATUS_WDF_POSTED";
        case 0xC0200200: return "STATUS_WDF_NO_PACKAGE";
        case 0xC0200201: return "STATUS_WDF_INTERNAL_ERROR";
        case 0xC0200203: return "STATUS_WDF_PAUSED";
        case 0xC0200204: return "STATUS_WDF_BUSY";
        case 0xC0200205: return "STATUS_WDF_IO_TIMEOUT_NOT_SENT";
        case 0xC0200206: return "STATUS_WDF_DEVICE_REMOVED_NOT_SENT";
        case 0xC0200207: return "STATUS_WDF_REQUEST_ALREADY_PENDING";
        case 0xC0200208: return "STATUS_WDF_REQUEST_INVALID_STATE";
        case 0xC0200209: return "STATUS_WDF_OBJECT_ATTRIBUTES_INVALID";
        case 0xC020020A: return "STATUS_WDF_TOO_FRAGMENTED";
        case 0xC020020B: return "STATUS_WDF_NO_CALLBACK";
        case 0xC020020C: return "STATUS_WDF_INCOMPATIBLE_EXECUTION_LEVEL";
        case 0xC020020D: return "STATUS_WDF_PARENT_ALREADY_ASSIGNED";
        case 0xC020020E: return "STATUS_WDF_PARENT_IS_SELF";
        case 0xC020020F: return "STATUS_WDF_PARENT_ASSIGNMENT_NOT_ALLOWED";
        case 0xC0200210: return "STATUS_WDF_SYNCHRONIZATION_SCOPE_INVALID";
        case 0xC0200211: return "STATUS_WDF_EXECUTION_LEVEL_INVALID";
        case 0xC0200212: return "STATUS_WDF_PARENT_NOT_SPECIFIED";

        default:
            RtlStringCbPrintfA(unknown, sizeof(unknown), "STATUS_UNKNOWN, code 0x%08x", Status);
            return unknown;
    }
}
...
{
    // special ascii table to be used for display ('.' instead of control characters
    // and upper half of standard ASCII table)
    static char ascii[256];
    static int ascii_initialized = 0;

    char *p;
    size_t i, l, k, o;
    char linebuffer[300], temp[100];

    // initialize ascii table
    if (ascii_initialized == 0)
    {
        for (i = 0; i <= 31; i++)
            ascii[i] = '.';
        for (i = 32; i <= 127; i++)
            ascii[i] = (char) (i & 0x000000ff);
        for (i = 128; i <= 255; i++)
            ascii[i] = '.';
    ...
    o = (size_t) PAddress;
    p = (char*) PAddress;
    KdPrint(("\nRaw dump size 0x%08x  address 0x%08Ix\n", DwLength, o));

    k = 0;

    // dump first partial line
    if ((o & 0x0000000f) != 0)
    {
        ...
        // dump position
        RtlStringCbPrintfA(linebuffer, sizeof(linebuffer), "0x%08Ix  ", o);

        // dump blank before hex
        for (i = 0; i < (o & 0x0f); i++)
        {
            RtlStringCbCatA(linebuffer, sizeof(linebuffer), "   ");
        }

        // dump hex
        for (i = 0; i < l; i++)
        {
            RtlStringCbPrintfA(temp, sizeof(temp), "%02hx ", p[i] & 0x000000ff);
            RtlStringCbCatA(linebuffer, sizeof(linebuffer), temp);
        }

        // dump blank before ascii
        RtlStringCbCatA(linebuffer, sizeof(linebuffer), "  ");
        for (i = 0; i < (o & 0x0f); i++)
        {
            RtlStringCbCatA(linebuffer, sizeof(linebuffer), " ");
        ...
        for (i = 0; i < l; i++)
        {
            RtlStringCbPrintfA(temp, sizeof(temp), "%1.1c", ascii[p[i]]);
            RtlStringCbCatA(linebuffer, sizeof(linebuffer), temp);
        }
